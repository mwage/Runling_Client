...
namespace Assets.Scripts.UI.RLR_Menu
{
    public class HighScoreMenuRLR : MonoBehaviour {
        ...
        private GameObject[] _levelScore = new GameObject[LevelManagerRLR.NumLevels];
        ...
        public void SetNumbers()
        {
            ...
            for (var i = 0; i < LevelManagerRLR.NumLevels; i++)
            {
                _levelScore[i].transform.GetChild(2).GetComponent<TextMeshProUGUI>().text = HighScoreRLR.highScoreRLR[i + 1].ToString();
            }

            _gameScore.transform.GetChild(2).GetComponent<TextMeshProUGUI>().text = HighScoreRLR.highScoreRLR[0].ToString();
            _combinedScore.transform.GetChild(2).GetComponent<TextMeshProUGUI>().text = HighScoreRLR.highScoreRLR[14].ToString();
            ...
            if (GameControl.GameActive)
            {
                for (var i = 0; i < LevelManagerRLR.NumLevels; i++)
                {
                    _levelScore[i].transform.GetChild(1).GetComponent<TextMeshProUGUI>().text = ScoreRLR.LevelScoreCurGame[i].ToString();
                }
